...
function createGridLines(bounds) {
  ...
  //--------computing the actual resolution/discretization steps--------
  llOffset_ = llOffset * selectedStep_;

  // define the size of the grid
  var topLat = Math.ceil(north / llOffset_) * llOffset_;
  var rightLong = Math.ceil(east / llOffset_) * llOffset_;

  var bottomLat = Math.floor(south / llOffset_) * llOffset_;
  var leftLong = Math.floor(west / llOffset_) * llOffset_;
  ...
  for (var latitude = bottomLat; latitude <= topLat; latitude += llOffset_) {
    // lines of latitude
    var latlngs = [
      [latitude, leftLong],
      [latitude, rightLong],
    ...
    if (layer instanceof L.Polyline && !(layer instanceof L.Polygon)) {
      ...
      if (
        !restrictedZones.some((zone) =>
          zone.getBounds().intersects(pathLine.getBounds())
        )
      ) {
  ...
  gridCoordinates.forEach((coord) => {
    ...
    missionZones.forEach(function (missionZone) {
      if (isMarkerInsidePolygon(marker, missionZone)) {
        if (
          !restrictedZones.some((zone) => isMarkerInsidePolygon(marker, zone))
        ) {
...
function lineSegmentsIntersect(line1, line2) {
  ...
  function ccw(l1p1, l1p2, l2p1) {
    return (
      (l2p1.lng - l1p1.lng) * (l1p2.lat - l1p1.lat) >
      (l1p2.lng - l1p1.lng) * (l2p1.lat - l1p1.lat)
    );
  }

  return (
    ccw(line1point1, line2point1, line2point2) !==
      ccw(line1point2, line2point1, line2point2) &&
    ccw(line1point1, line1point2, line2point1) !==
      ccw(line1point1, line1point2, line2point2)
  );
}
...
function layerGroupClickHandler(event) {
  ...
  if (pointOption == 'start-point') {
    ...
    var markerStart = L.marker([markerPoistion.lat, markerPoistion.lng], {
      icon: startIcon,
    });
...
function distance(missionpathPoint1, missionpathPoint2) {
  const [x1, y1] = missionpathPoint1;
  const [x2, y2] = missionpathPoint2;
...
function drawPath() {
  ...
  //--------check each layer in the group layers--------
  missionPoints.eachLayer(function (layer) {
    //--------check for getLatLng method in case of marker--------
    if (layer.getLatLng) {
      let latlng = layer.getLatLng();
      pathLats.push(latlng.lat);
      pathLongs.push(latlng.lng);
    }
    ...
    //--------check condition for other layers if required--------
  });
  //--------combine latitudes-longitudes to from a path--------
  for (let i = 0; i < pathLats.length; i++) {
    missionpath.push([pathLats[i], pathLongs[i]]);
  }
  let test = missionpath;

  // Nearest neighbor algorithm
  function nearestNeighbor(missionpath) {
    const n = missionpath.length;
    const visited = Array(n).fill(false);
    const path = [0]; // Start from the first point
    visited[0] = true;

    for (let i = 0; i < n - 1; i++) {
      const lastPoint = missionpath[path[path.length - 1]];
      let minDist = Infinity;
      let nearestIndex = null;

      for (let j = 0; j < n; j++) {
        if (!visited[j]) {
          const dist = distance(lastPoint, missionpath[j]);
          if (dist < minDist) {
            minDist = dist;
            nearestIndex = j;
          }
  ...
  // Get the order of points using the nearest neighbor algorithm
  let order = nearestNeighbor(missionpath);

  // Draw the path on the map
  pathCoordinates_ = order.map((index) => missionpath[index]);

  // Output the order of points
  /*console.log('Order of points:', order);
  console.log('Not ordered points:', test);
  console.log('Order of coordin:', pathCoordinates_);*/

  linepathCoordinates_ = L.polyline(pathCoordinates_, {
    color: 'green',
    smoothFactor: 0.5,
  }).addTo(map);
  console.log('checkdata:', linepathCoordinates_);
  // Optionally fit the map bounds to the polyline
  map.fitBounds(linepathCoordinates_.getBounds());
}
