...
let latGridValues_ = [];
let lngGridValues_ = [];
let gridCoordinates_ = [];
let startPoint = L.marker();
let clickedLocations = [];
let polygon;
let missionZones = [];
let restrictedZones = [];
let gridCoordinates = []; // Grid points
let missionPoints = L.layerGroup();
let pathCoordinates_ = [];
let linepathCoordinates_ = L.layerGroup();
let gridLayer = L.layerGroup();
let llOffset = 0.0000141003948; //0.0000666666666667; // generate a grid with 4.728m between nodes using haversine formula
let llOffset_;
let selectedStep_ = 1.0;
let drawGridBox = false;
...
function getDiscretizationstep() {
  ...
  selectedStep =
    step === ''
      ? Number(placeHolder) + ' ' + 'meter/s'
      : step + ' ' + 'meter/s';
  selectedStep_ = step === '' ? Number(placeHolder) : step;
...
exportJSON.addEventListener('click', () => {
  ...
  const convertedData =
    ...
    encodeURIComponent(JSON.stringify(pathCoordinates_));
...
saveJSON.addEventListener('click', (e) => {
  ...
  localStorage.setItem('geojson', JSON.stringify(pathCoordinates_));
});
...
removeJSON.addEventListener('click', (e) => {
  ...
  drawnItems.eachLayer(function (layer) {
    ...
    clearAll();
    map.removeLayer(linepathCoordinates_);
    map.removeLayer(missionPoints);
  });
});
